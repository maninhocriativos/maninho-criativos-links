...
- **Envelope de abertura:** overlay `#gate` com envelope animado em CSS (aba abre, carta sobe, zoom e fade) que revela a proposta ao clicar no envelope ou no botão "Abrir proposta". A lógica vive em um `<script>` próprio e à prova de falhas (se algo quebrar, a proposta aparece em vez de travar no envelope). O envelope aparece **sempre** ao carregar, mesmo com `#seção` na URL (o hash é lido, removido da URL e usado para rolar até a seção depois de abrir). Para pular o envelope (testes/screenshots): `/?direto`. Respeita `prefers-reduced-motion`.
